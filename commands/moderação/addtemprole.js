...
function loadTempRoles() {
    if (!fs.existsSync(TEMP_ROLES_PATH)) {
        fs.writeFileSync(TEMP_ROLES_PATH, '[]', 'utf8');
        return [];
    }
    try {
        const data = fs.readFileSync(TEMP_ROLES_PATH, 'utf8');
...
module.exports = {
    ...
    async execute(client, interactionOrMessage, args) {
        ...
        const reply = async (options) => {
            ...
            if (isSlash) {
                if (interactionOrMessage.replied || interactionOrMessage.deferred) return interactionOrMessage.followUp(finalOptions);
        ...
        let roleNameQuery, durationString;
        if (isSlash) {
            roleNameQuery = interactionOrMessage.options.getString('nome_do_cargo');
            durationString = interactionOrMessage.options.getString('duracao');
        } else {
            if (args.length < 3) return reply({ content: '❌ Sintaxe incorreta. Use: `!addtemprole @usuario <nome do cargo> <duração>`' });
            durationString = args[args.length - 1];
            roleNameQuery = args.slice(1, -1).join(' ');
        }
        ...
        if (foundRoles.size > 1) {
            const roleList = foundRoles.map((r, i) => `${i + 1} - ${r.name}`).join('\n');
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(`O cargo \`${role.name}\` foi adicionado a **${targetMember.user.tag}**.`)
